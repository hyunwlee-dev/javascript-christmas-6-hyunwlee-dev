...
- 예외 발생 시
  ...
  - [ ] 에러메세지는 `[Error]`로 시작해야 합니다.
- [ ] 에러메세지 출력 후 재입력을 요구합니다.
- 방문 날짜

  - [ ] `VisitDate`를 인스턴스화 시킵니다.
- 주문 메뉴와 주문 개수

  - [ ] `MenuQunantity`를 인스턴스화 시킵니다.
- 주문
  - [ ] `Order`를 인스턴스화 시킵니다.
...
- 방문 날짜 입력
  ...
  - [ ] `MissionUtils.Console.readLineAsync()`으로 입력
- 주문 메뉴와 메뉴 개수 입력
...
- 이벤트 혜택 미리보기 문구
  ...
  - [ ] `12월 26에 우테코 식당에서 받을 이벤트 혜택 미리 보기!` 출력
